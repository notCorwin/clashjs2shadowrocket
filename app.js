...
const routeDescriptions = { Apple: "Apple 服务直连", AI: "Claude、ChatGPT、Gemini 等 AI 服务", LINE: "LINE 服务", Netflix: "Netflix 与 Fast.com", YouTube: "YouTube 视频服务", TikTok: "TikTok 短视频服务", Google: "Google 服务", "局域网直连": "局域网、保留地址与特殊网段" };
...
function setupGenerator(data) {
  ...
  const renderRoutes = () => { document.querySelector("#route-options").innerHTML = data.routes.map((route, index) => { const count = routeDomains(data, route).length + (route.keywords || []).length + (route.cidrs || []).length + (route.geoips || []).length; return `<div class="route-option"><div><input type="checkbox" aria-label="启用 ${escapeHtml(route.name)}" data-route="${index}" ${route.enabled !== false ? "checked" : ""}><strong>${escapeHtml(route.name)}</strong></div><em>${routeDescriptions[route.name] || "自定义分流规则"} · ${count} 条匹配规则</em><select data-target-route="${index}" aria-label="${escapeHtml(route.name)} 导向节点组">${Object.entries(data.targets).map(([key, name]) => `<option value="${escapeHtml(key)}" ${route.target === key ? "selected" : ""}>${escapeHtml(name)}</option>`).join("")}</select><button type="button" class="edit-domains" data-edit-route="${index}">编辑网址列表</button></div>`; }).join("");
    ...
    document.querySelectorAll("[data-edit-route]").forEach((button) => button.onclick = () => { editingRoute = Number(button.dataset.editRoute); const route = data.routes[editingRoute]; const domains = routeDomains(data, route); const domainTargets = route.domain_targets || route._domainTargets || []; document.querySelector("#drawer-title").textContent = `${route.name} · 网址列表`; document.querySelector("#drawer-items").innerHTML = domains.map((domain, index) => `<div class="drawer-item"><input aria-label="${escapeHtml(route.name)} 网址 ${index + 1}" data-drawer-domain="${index}" value="${escapeHtml(domain)}"><select aria-label="${escapeHtml(domain)} 导向节点组" data-drawer-target="${index}">${Object.entries(data.targets).map(([key, name]) => `<option value="${escapeHtml(key)}" ${(domainTargets[index] || route.target) === key ? "selected" : ""}>${escapeHtml(name)}</option>`).join("")}</select></div>`).join(""); document.querySelector("#drawer-backdrop").classList.remove("hidden"); document.querySelector("#domain-drawer").classList.add("open"); });
  };
  renderRoutes();
  ...
  const renderRules = () => { document.querySelector("#rule-options").innerHTML = (data.standalone_rules || []).map((item, index) => { const type = item.cidrs?.length ? "cidrs" : item.geoips?.length ? "geoips" : "domains"; return `<div class="standalone-rule"><input aria-label="规则名称" data-rule-name="${index}" value="${escapeHtml(item.name)}"><select aria-label="规则类型" data-rule-type="${index}"><option value="cidrs" ${type === "cidrs" ? "selected" : ""}>CIDR / IP</option><option value="geoips" ${type === "geoips" ? "selected" : ""}>GeoIP</option><option value="domains" ${type === "domains" ? "selected" : ""}>域名</option></select><textarea aria-label="规则值" data-rule-values="${index}" spellcheck="false">${escapeHtml((item[type] || []).join("\n"))}</textarea><select aria-label="目标节点组" data-rule-target="${index}">${Object.entries(data.targets).map(([key, name]) => `<option value="${escapeHtml(key)}" ${item.target === key ? "selected" : ""}>${escapeHtml(name)}</option>`).join("")}</select><button type="button" data-remove-rule="${index}">删除</button></div>`; }).join(""); document.querySelectorAll("[data-rule-name]").forEach((input) => input.oninput = () => { data.standalone_rules[input.dataset.ruleName].name = input.value; }); document.querySelectorAll("[data-rule-target]").forEach((select) => select.onchange = () => { data.standalone_rules[select.dataset.ruleTarget].target = select.value; }); document.querySelectorAll("[data-remove-rule]").forEach((button) => button.onclick = () => { data.standalone_rules.splice(Number(button.dataset.removeRule), 1); renderRules(); }); };
  data.standalone_rules ||= []; renderRules();
  const syncRule = (index) => { const area = document.querySelector(`[data-rule-values="${index}"]`); const type = document.querySelector(`[data-rule-type="${index}"]`).value; data.standalone_rules[index][type] = area.value.split(/\r?\n/).map((value) => value.trim()).filter(Boolean); };
  ...
  document.querySelector("#rule-options").onchange = (event) => { if (event.target.matches("[data-rule-type]")) renderRules(); };
  ...
  document.querySelector("#save-domains").onclick = () => { if (editingRoute === null) return; const route = data.routes[editingRoute]; const domains = [...document.querySelectorAll("[data-drawer-domain]")]; const targets = [...document.querySelectorAll("[data-drawer-target]")]; const entries = domains.map((input, index) => ({ domain: input.value.trim(), target: targets[index]?.value || route.target })).filter((entry) => entry.domain); route.domain_sets = []; route.domains = entries.map((entry) => entry.domain); route.domain_targets = entries.map((entry) => entry.target); delete route._domainTargets; document.querySelector("#drawer-backdrop").click(); renderRoutes(); };
